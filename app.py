...
# Mutable application data can live on a Render persistent disk when
# MANDI_DATA_DIR is set (for example: /var/data). Locally it stays beside app.py.
DATA_DIR = os.environ.get("MANDI_DATA_DIR", BASE_DIR)
os.makedirs(DATA_DIR, exist_ok=True)
# Profile 1 keeps the original database name so existing data is preserved.
LEGACY_DB_PATH = os.path.join(DATA_DIR, "mandi.db")
PROFILE_REGISTRY_DB = os.path.join(DATA_DIR, "profiles.db")
...
def init_user_store():
    """Create the global login store in profiles.db. Passwords are never stored in plain text."""
    con = registry_db()
    con.execute("""
...
def user_count():
    con = registry_db()
    row = con.execute("SELECT COUNT(*) AS n FROM app_users").fetchone()
    con.close()
    return int(row["n"] or 0)

def get_user(user_id):
    con = registry_db()
    row = con.execute("SELECT * FROM app_users WHERE id=?", (user_id,)).fetchone()
    con.close()
    return row

def get_user_by_username(username):
    con = registry_db()
    row = con.execute("SELECT * FROM app_users WHERE lower(username)=lower(?)", (username,)).fetchone()
    con.close()
    return row

def get_all_users():
    con = registry_db()
    rows = con.execute("SELECT * FROM app_users ORDER BY id").fetchall()
    con.close()
    return rows
...
def require_login():
    ...
    public = {"auth_choice", "login", "signup", "setup_user", "health", "manifest", "service_worker", "static"}
    if endpoint in public:
        return None
    if user_count() == 0:
        return None
    if not session.get("user_id") or current_user() is None or not current_user()["active"]:
        session.clear()
...
def db_path_for_profile(profile):
    return os.path.join(DATA_DIR, profile["db_filename"])
...
def settings_path(): return os.path.join(DATA_DIR,'settings.json')
def load_settings():
    try:
        with open(settings_path(),'r',encoding='utf-8') as f: return json.load(f)
    except Exception: return {'auto_sync':False,'sync_folder':''}
def save_settings(v):
...
@app.route("/auth")
def auth_choice():
    if session.get("user_id") and current_user() is not None and current_user()["active"]:
        return redirect(url_for("dashboard"))
    return render_template("auth_choice.html", has_users=user_count() > 0)
...
def signup():
    ...
    if request.method == "POST":
        ...
        if not re.fullmatch(r"[A-Za-z0-9_.-]{3,40}", username):
            flash("Username must be 3–40 characters and use only letters, numbers, dot, underscore or hyphen.", "error")
        elif len(password) < 8:
            flash("Password must be at least 8 characters.", "error")
        elif password != confirm:
            flash("Passwords do not match.", "error")
        elif get_user_by_username(username):
            flash("That username already exists. Please use another username or sign in.", "error")
        else:
            con = registry_db()
            try:
                # The first registered account becomes the owner/admin. All later self-signups are viewers.
                ...
                session["user_id"] = uid
                flash("Account created successfully. Welcome to Mandi Tracker!", "success")
                return redirect(url_for("dashboard"))
...
def login():
    ...
    if request.method == "POST":
        ...
        if user and user["active"] and check_password_hash(user["password_hash"], password):
            ...
            con = registry_db()
            con.execute("UPDATE app_users SET last_login=CURRENT_TIMESTAMP WHERE id=?", (user["id"],))
            con.commit()
            con.close()
            flash(f"Welcome back, {user['username']}.", "success")
            return redirect(url_for("dashboard"))
        flash("Incorrect username or password.", "error")
    return render_template("login.html", has_users=user_count() > 0)
...
def toggle_user(user_id):
    ...
    if user["id"] == me["id"]:
        ...
        return redirect(url_for("users"))
    con = registry_db()
    con.execute("UPDATE app_users SET active=? WHERE id=?", (0 if user["active"] else 1, user_id))
    con.commit()
    con.close()
    flash(f"User '{user['username']}' is now {'active' if not user['active'] else 'inactive'}.", "success")
    return redirect(url_for("users"))
...
def reset_user_password(user_id):
    ...
    if user is None:
        ...
        con = registry_db()
        con.execute("UPDATE app_users SET password_hash=? WHERE id=?", (generate_password_hash(password), user_id))
        con.commit()
        con.close()
...
def delete_user(user_id):
    ...
    if user["role"] == "admin":
        con = registry_db()
        admins = con.execute("SELECT COUNT(*) AS n FROM app_users WHERE role='admin' AND active=1").fetchone()["n"]
        con.close()
        if admins <= 1:
...
def export_daily_moong():
    ...
    out = os.path.join(DATA_DIR, "daily_moong_average_analysis.xlsx")
...
def export_daily_moong_day(day):
    ...
    out = os.path.join(DATA_DIR, f"daily_moong_report_{safe_day}.xlsx")
    df.to_excel(out, index=False)
...
def export_daily_grain():
    ...
    """, con)
    con.close()
    out = os.path.join(DATA_DIR, "daily_grain_variety_purchase_average.xlsx")
    df.to_excel(out, index=False)
...
def export_daily_grain_day(day):
    ...
    con.close()
    safe_day = re.sub(r"[^0-9A-Za-z_-]+", "_", day) or "day"
    out = os.path.join(DATA_DIR, f"daily_report_{safe_day}.xlsx")
    df.to_excel(out, index=False)
...
def api_export_selected():
    ...
    con.close()
    out = os.path.join(DATA_DIR, "mandi_selected_export.xlsx")
    df.to_excel(out, index=False)
...
def api_export():
    ...
    con.close()
    out=os.path.join(DATA_DIR,"mandi_purchase_export.xlsx")
    df.to_excel(out,index=False)
    return redirect(url_for("download_export"))

@app.route("/download-export")
def download_export():
    return send_file(os.path.join(DATA_DIR,"mandi_purchase_export.xlsx"), as_attachment=True)
...
def export_pdf():
    ...
    out=os.path.join(DATA_DIR,'mandi_purchase_report.pdf'); doc=SimpleDocTemplate(out,pagesize=landscape(A4)); st=getSampleStyleSheet(); story=[Paragraph('Mandi Purchase Report',st['Title']),Spacer(1,12)]
...
def backup():
    ...
    out=os.path.join(DATA_DIR,f"mandi_backup_profile_{profile['id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip")
...
def settings_page():
    st=load_settings()
    if request.method=='POST':
        st['sync_folder']=(request.form.get('sync_folder') or '').strip(); st['auto_sync']=bool(request.form.get('auto_sync')); save_settings(st); flash('Settings saved.','success'); return redirect(url_for('settings_page'))
...
@app.route('/health')
def health():
    """Simple Render HTTP health check."""
    try:
        con = sqlite3.connect(PROFILE_REGISTRY_DB)
        con.execute('SELECT 1').fetchone()
        con.close()
        return jsonify({"ok": True, "service": "mandi-purchase-tracker"})
...
def inject_profile_context():
    return {"active_profile": current_profile(), "all_profiles": get_profiles(), "current_user": current_user()}
